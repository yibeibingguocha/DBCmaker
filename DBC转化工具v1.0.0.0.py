...
root = tk.Tk()
root.geometry("720x520")  # 紧凑布局，刚好容纳所有控件
root.protocol("WM_DELETE_WINDOW", on_closing)
root.title("DBC转化工具 v1.0.0.0")

# 配置grid权重，使窗口可调整大小
root.grid_rowconfigure(0, weight=1)  # 日志区可垂直扩展
root.grid_rowconfigure(1, weight=0)  # 输入区域固定高度
...
input_message_name_entry = tk.Entry(input_frame)
...
input_message_id_entry = tk.Entry(input_frame)
...
input_signal_name_entry = tk.Entry(input_frame)
...
input_start_bit_entry = tk.Entry(input_frame)
...
input_length_entry = tk.Entry(input_frame)
...
input_precision_entry = tk.Entry(input_frame)
...
input_offset_entry = tk.Entry(input_frame)
...
input_comment_entry = tk.Entry(input_frame)
...
input_direction_entry = tk.Entry(input_frame)
...
input_cycle_time_entry = tk.Entry(input_frame)
...
input_frame_format_entry = tk.Entry(input_frame)
input_frame_format_entry.insert(0, "Frame Format")  # 设置默认值
...
input_message_length_entry = tk.Entry(input_frame)
input_message_length_entry.insert(0, "Msg Length(Byte)")  # 设置默认值
